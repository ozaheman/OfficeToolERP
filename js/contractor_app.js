...
// --- START OF MODIFIED contractor_app.js (site_app.js logic) ---
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function toggleTabsForView(isProjectView) {
        ...
        const tabsToToggle = ['status', 'project-docs', 'tender-docs', 'vendor-lists', 'materials', 'subcontractors', 'long-lead', 'rfi', 'project-uploads', 'bulletin', 'site-docs', 'boq', 'schedule', 'tools', 'forms'];
    ...
    async function handleProjectSelect(event) {
        ...
        if (!siteData) {
            ...
            siteData = { jobNo: currentJobNo, status: 'Pending Start', progress: 0, boq: boqTemplate, mom: [], paymentCertificates: [], statusLog: [], scheduleOverrides: [], overlaySchedule: [] };
        ...
        // MODIFICATION: Show/Hide the force render button based on project type
        if (project.projectType && project.projectType.toLowerCase() !== 'villa') {
            DOMElements.forceRenderScheduleBtn.style.display = 'inline-block';
        } else {
            DOMElements.forceRenderScheduleBtn.style.display = 'none';
        }
        ...
        await renderMomTaskFollowup(); // New: Show follow-up tasks
    ...
    // MODIFICATION: Logic for when to render the chart is now handled here.
    async function renderGanttChart(force = false) {
        if (!currentJobNo || !window.UrbanAxisSchedule || !window.OverlayGantt) return;
        
        const project = await DB.getProject(currentJobNo);
        const siteData = await DB.getSiteData(currentJobNo);
        const baselineSchedule = await getProjectSchedule(project, siteData);
        const overlaySchedule = siteData.overlaySchedule || [];
        const analyzedOverlaySchedule = calculateCriticalPath(overlaySchedule, 'uid');

        const container = document.getElementById('villa-schedule-preview');
        
        // Logic to decide if the baseline chart should be rendered
        if (force || (project.projectType && project.projectType.toLowerCase() === 'villa')) {
            alert('render schedule');
            UrbanAxisSchedule.render(baselineSchedule, handleGanttUpdate);
        } else {
            container.innerHTML = '<div style="padding: 40px; text-align: center; color: #777;">Schedule available for "Villa" type projects. Use "Force Render" to display a template-based schedule.</div>';
        }

        // The advanced chart always tries to render what it's given
        const advancedContainer = document.getElementById('advanced-schedule-view');
        OverlayGantt.render(advancedContainer, baselineSchedule, analyzedOverlaySchedule, handleOverlayGanttUpdate);
    }
    ...
    // NEW: Handle updates from the overlay chart (e.g., double-click to edit)
    async function handleOverlayGanttUpdate(update) {
        if (update.action === 'edit' && update.task) {
            openOverlayTaskModal(update.task.uid);
        }
    }

    async function getProjectSchedule(projectData, siteData) {
        const projectStartDate = new Date(projectData.agreementDate);
        // MODIFICATION: Provide a default construction duration to prevent errors
    ...
    // MODIFIED: Added `idKey` to handle different unique identifiers (`id` vs `uid`)
    function calculateCriticalPath(schedule, idKey = 'id') {
        if (!schedule || schedule.length === 0) return schedule;

        const tasks = JSON.parse(JSON.stringify(schedule));
        const taskMap = new Map(tasks.map(task => [task[idKey], task]));
        ...
        tasks.forEach(task => {
            task.dependencies.forEach(depId => {
                ...
                if (predecessor) { predecessor.successors.push(task[idKey]); }
            });
        });
        ...
        tasks.forEach(task => {
            ...
            task.isCritical = task.isCritical || (task.slack <= 1); // Respect manual override
        });
    ...
    async function handleGenerateProjectReport() {
        ...
        try {
            ...
            // Use overlay schedule for analysis if available, otherwise baseline
            const scheduleForAnalysis = (siteData.overlaySchedule && siteData.overlaySchedule.length > 0) 
                ? calculateCriticalPath(siteData.overlaySchedule, 'uid')
                : await getProjectSchedule(project, siteData).then(s => calculateCriticalPath(s));
                
            const reportHtml = await generateReportHtml(project, siteData, scheduleForAnalysis);
    ...
    // --- NEW OVERLAY GANTT LOGIC ---
    async function openOverlayTaskModal(taskUid = null) {
        if (!currentJobNo) return;
        const siteData = await DB.getSiteData(currentJobNo);
        const schedule = siteData.overlaySchedule || [];
        const isNew = taskUid === null;
        
        DOMElements.overlayTaskModalTitle.textContent = isNew ? "Add Detailed Task" : "Edit Detailed Task";
        DOMElements.overlayTaskEditId.value = isNew ? '' : taskUid;
        DOMElements.deleteOverlayTaskBtn.style.display = isNew ? 'none' : 'inline-block';

        // Populate dependencies dropdown
        const depSelect = DOMElements.overlayTaskDependencies;
        depSelect.innerHTML = schedule.filter(t => t.uid !== taskUid).map(t => `<option value="${t.uid}">${t.name}</option>`).join('');

        if (isNew) {
            // Reset form
            const form = DOMElements.overlayTaskModal.querySelector('.modal-body');
            form.querySelectorAll('input[type="text"], input[type="date"], input[type="number"], textarea').forEach(el => el.value = '');
            form.querySelector('input[type="checkbox"]').checked = false;
            DOMElements.overlayTaskDuration.value = 1;
            depSelect.value = null;

        } else {
            const task = schedule.find(t => t.uid === taskUid);
            if (!task) { alert('Task not found'); return; }
            DOMElements.overlayTaskName.value = task.name || '';
            DOMElements.overlayTaskType.value = task.type || '';
            DOMElements.overlayTaskStart.value = task.start || '';
            DOMElements.overlayTaskDuration.value = task.duration || 1;
            DOMElements.overlayTaskResponsible.value = task.responsible || '';
            DOMElements.overlayTaskVendor.value = task.vendor || '';
            DOMElements.overlayTaskLabor.value = task.labor || '';
            DOMElements.overlayTaskMaterials.value = task.materials || '';
            DOMElements.overlayTaskReason.value = task.reason || '';
            DOMElements.overlayTaskIsCritical.checked = task.isCritical || false;
        ...
        DOMElements.overlayTaskModal.style.display = 'flex';
    }

    async function saveOverlayTask() {
        if (!currentJobNo) return;
        const siteData = await DB.getSiteData(currentJobNo);
        siteData.overlaySchedule = siteData.overlaySchedule || [];

        const uid = DOMElements.overlayTaskEditId.value;
        const duration = parseInt(DOMElements.overlayTaskDuration.value, 10) || 1;
        const startDate = new Date(DOMElements.overlayTaskStart.value + 'T00:00:00');
        const endDate = new Date(startDate);
        endDate.setDate(startDate.getDate() + duration - 1);

        const dependencies = Array.from(DOMElements.overlayTaskDependencies.selectedOptions).map(opt => opt.value);

        const taskData = {
            uid: uid || `task_${Date.now()}`,
            id: uid ? siteData.overlaySchedule.find(t=>t.uid===uid).id : null, // Preserve baseline link if editing
            name: DOMElements.overlayTaskName.value,
            type: DOMElements.overlayTaskType.value,
            start: DOMElements.overlayTaskStart.value,
            duration: duration,
            end: endDate.toISOString().split('T')[0],
            responsible: DOMElements.overlayTaskResponsible.value,
            vendor: DOMElements.overlayTaskVendor.value,
            labor: DOMElements.overlayTaskLabor.value,
            materials: DOMElements.overlayTaskMaterials.value,
            dependencies: dependencies,
            reason: DOMElements.overlayTaskReason.value,
            isCritical: DOMElements.overlayTaskIsCritical.checked
        };
        
        if (uid) { // Editing existing task
            const index = siteData.overlaySchedule.findIndex(t => t.uid === uid);
            if (index > -1) siteData.overlaySchedule[index] = taskData;
        } else { // New task
            siteData.overlaySchedule.push(taskData);
        }

        await DB.putSiteData(siteData);
        DOMElements.overlayTaskModal.style.display = 'none';
        await renderGanttChart();
    }

    async function deleteOverlayTask() {
        if (!currentJobNo || !confirm("Are you sure you want to delete this task?")) return;
        const uid = DOMElements.overlayTaskEditId.value;
        if (!uid) return;

        const siteData = await DB.getSiteData(currentJobNo);
        siteData.overlaySchedule = (siteData.overlaySchedule || []).filter(t => t.uid !== uid);
