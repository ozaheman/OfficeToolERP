...
const UrbanAxisSchedule = (() => {
    ...
    const drawChart = (container, schedule, onUpdate) => {
        container.innerHTML = '';
        if (!schedule || !Array.isArray(schedule) || schedule.length === 0) { // Added isArray check for safety
             container.innerHTML = '<div style="padding: 40px; text-align: center; color: #888;">No schedule data available to display.</div>';
            return;
        }
     ...
     * CORRECTED: The function now accepts the schedule data directly.
     */
    const render = (scheduleData, onUpdate) => {
        const container = document.getElementById('villa-schedule-preview');
        if (!container) {
            console.error("Gantt chart container 'villa-schedule-preview' not found.");
            return;
        }
        
        try {
            drawChart(container, scheduleData, onUpdate);
        } catch (error) {
            console.error("Failed to render Gantt chart:", error);
            container.innerHTML = '<div style="padding: 40px; text-align: center; color: red;">An error occurred while generating the schedule. Please check console for details.</div>';
        }
    };
